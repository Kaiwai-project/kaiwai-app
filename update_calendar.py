...
def analyze_post(llm: anthropic.Anthropic, brand_name: str, text: str, today: datetime) -> dict | None:
    """LLM으로 SNS 게시물에서 이벤트를 추출. 이벤트 없으면 None 반환."""
...
def main():
    ...
    for brand in brands:
        ...
        for i, text in enumerate(texts):
            ...
            if llm:
                result = analyze_post(llm, brand["name"], text, today)
                if result:
